...
export function V31EditorialPage({lang="el",kind}:{lang?:Lang;kind:Kind}){
 ...
 return <V31PageFrame lang={lang}>
  ...
  {kind==="guides"?<section className="wf-section wf-section--tight"><div className="wf-shell"><a href="/guides/hidden-greece" className="wf-card wf-card--dark" style={{display:"block",padding:"clamp(28px,5vw,58px)",textDecoration:"none"}}><span className="wf-kicker wf-kicker--light">FEATURED · VOLUME 01 · HIDDEN GREECE</span><h2 className="wf-h2" style={{color:"white",marginTop:12}}>{say(lang,"Μέρη που δεν ήξερες ότι χρειάζεσαι.","Places you didn’t know you needed.")}</h2><p className="wf-body wf-footer-copy" style={{maxWidth:760}}>{say(lang,"12 λιγότερο προφανείς αποδράσεις που λύνουν πραγματικά travel pain gaps — από μία απρόσμενη μονοήμερη μέχρι ελληνική alpine wilderness.","12 less-obvious escapes built around real travel pain gaps — from an unexpected day trip to Greek alpine wilderness.")}</p><span className="wf-btn wf-btn--secondary" style={{marginTop:18}}>{say(lang,"Άνοιξε το Hidden Greece →","Open Hidden Greece →")}</span></a></div></section>:null}
