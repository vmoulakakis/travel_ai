...
export function V50StayFunnel({slug,destination,destinationEn,offer,trip,lang}:Props){
 ...
 async function sendChat(e:FormEvent){e.preventDefault();const q=chatInput.trim();if(!q||agentBusy)return;setChat(v=>[...v,{role:"user",text:q}]);setChatInput("");setAgentBusy(true);try{const r=await fetch("/api/escape/agent-chat",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({trip,slug,offerId:offer.sourceProductId,message:q})});const d=await r.json().catch(()=>null) as {reply?:string}|null;setChat(v=>[...v,{role:"agent",text:d?.reply||say(lang,"Κρατάω το stay και το πλάνο σου ενεργά. Πες μου τι θέλεις να αλλάξουμε και θα συνεχίσουμε από εδώ.","I’m keeping your stay and trip plan active. Tell me what you want to change and we’ll continue from here.")}])}catch{setChat(v=>[...v,{role:"agent",text:say(lang,"Κρατάω το stay και το πλάνο σου ενεργά. Πες μου τι θέλεις να αλλάξουμε και θα συνεχίσουμε από εδώ.","I’m keeping your stay and trip plan active. Tell me what you want to change and we’ll continue from here.")}])}finally{setAgentBusy(false)}}
 async function sendEmail(){if(!/^\S+@\S+\.\S+$/.test(email)){setEmailMsg(say(lang,"Βάλε έγκυρο email.","Enter a valid email."));return}try{const r=await fetch("/api/guide/email",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({email,slug,start:trip.startDate,end:trip.endDate,offer:offer.sourceProductId,language:lang,trip})});if(r.ok){setUnlocked(true);setEmailMsg(say(lang,"Το Escape Book στάλθηκε και ξεκλείδωσε το τελικό booking CTA.","Escape Book sent and the final booking CTA is unlocked."))}else setEmailMsg(say(lang,"Το email δεν στάλθηκε αυτή τη στιγμή. Το ταξίδι σου παραμένει διαθέσιμο εδώ.","The email was not sent right now. Your trip remains available here."))}catch{setEmailMsg(say(lang,"Το email δεν στάλθηκε αυτή τη στιγμή. Το ταξίδι σου παραμένει διαθέσιμο εδώ.","The email was not sent right now. Your trip remains available here."))}}
