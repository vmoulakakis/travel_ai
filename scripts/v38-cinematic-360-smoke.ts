...
const home=read("app/page.tsx"),homeEn=read("app/en/page.tsx"),v40=home.includes("V40DiscoveryExperience"),funnel=read(v40?"components/v40-discovery-experience.tsx":"components/v38-escape-funnel.tsx"),builder=read(v40?"components/v40-stay-workspace.tsx":"components/v38-escape-builder-client.tsx"),parser=read("lib/data/destination-v8.ts"),local=read("lib/data/local-intelligence-v38.ts"),research=read("app/api/escape/research/route.ts"),guide=read("app/api/guide/route.ts"),email=read("app/api/guide/email/route.ts"),feedback=read("app/api/escape/feedback/route.ts"),skill=read("skills/web-design/SKILL.md"),priceMigration=read("supabase/migrations/20260915165500_v38_stay_price_truth_rpc.sql");
const failures:string[]=[];const expect=(ok:boolean,message:string)=>{if(!ok)failures.push(message)};
expect(v40?(home.includes("V40DiscoveryExperience")&&homeEn.includes("V40DiscoveryExperience")):(home.includes("V38EscapeFunnel")&&homeEn.includes("V38EscapeFunnel")),"current GR + EN home routes must use the active discovery experience");
expect(v40?(funnel.includes("Travel Agent")&&funnel.includes("nextQuestion")&&funnel.includes("Destination reveal")&&funnel.includes("solve-v36/stream")):funnel.includes('type Stage="feel"|"when"|"reality"|"solving"|"results"'),"active discovery must keep the guided decision funnel and real dual-pass engine");
expect(funnel.includes("mode=aerial"),"active discovery must keep sourced aerial destination imagery");
...
expect(v40?(builder.includes("stayImages")&&builder.includes("extra_images")&&builder.includes("mode=aerial")):(builder.includes("mode=aerial")&&builder.includes("setInterval")&&builder.includes("stayImages")),"destination and stay sourced-photo cinematic system must remain wired");
...
expect(v40?(builder.includes("unlocked")&&builder.includes("/api/guide/email")&&builder.includes("offer.trackingUrl")):(builder.includes('stage==="email"')&&builder.includes('stage==="unlocked"')),"affiliate handoff must stay behind successful email unlock");
...
expect(skill.includes("travel-ai-v38-web-design")&&skill.includes("Reference ledger")&&skill.includes("AI Guest Signal")&&skill.includes("Price truth"),"project-specific web-design truth rules must remain codified");
if(failures.length){console.error("V38 truth/cinematic compatibility smoke FAILED\n- "+failures.join("\n- "));process.exit(1)}
console.log(`V38_TRUTH_CONTRACTS_OK active=${v40?"V41-on-V40-shell":"V38"} media=sourced local=multi-source price=truthful feedback=sample-gated pdf=affiliate-disclosed email=gated`);
