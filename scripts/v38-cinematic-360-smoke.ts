...
expect(v40?(funnel.includes("Travel Agent")&&funnel.includes("3 προορισμοί")&&funnel.includes("solve-v36/stream")):funnel.includes('type Stage="feel"|"when"|"reality"|"solving"|"results"'),"active discovery must keep the guided decision funnel and real dual-pass engine");
...
console.log(`V38_TRUTH_CONTRACTS_OK active=${v40?"V40":"V38"} media=sourced local=multi-source price=truthful feedback=sample-gated pdf=affiliate-disclosed email=gated`);
