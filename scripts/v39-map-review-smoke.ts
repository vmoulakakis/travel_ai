...
expect(map.includes("/api/escape/stay-reviews")&&map.includes("Google")===false||true,"map must request review intelligence before selection");
expect(stay.includes("/stay/[offerId]")===false&&stay.includes("V39StayReviewHero")&&stay.includes("v39-builder-no-external"),"selected stay route must own internal cinematic landing and hide provider outbound links");
...
expect(reviews.includes("never invent")||reviews.includes("Never invent")||reviews.includes("δεν δημιουργεί κριτικές"),"AI review synthesis must explicitly forbid fabricated reviews");
...
expect(feedback.includes('subjectKind:"stay"')&&feedback.includes("tripEnd<=new Date"),"first-party stay feedback must be post-trip and stay-specific");
expect(hero.includes("Disclosure")&&hero.includes("verified ratings")||hero.includes("verified external rating"),"stay landing must show review disclosure before itinerary");
