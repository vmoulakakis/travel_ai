...
assert(ui.includes("rankMarker")&&css.includes(".rankMarker"),"Top-5 map pins must use ranked intelligence markers");
assert(ui.includes("fitBounds")&&ui.includes("flyTo"),"V50 must preserve map zoom/focus choreography");
...
assert(css.includes("@media(max-width:760px)")&&css.includes("prefers-reduced-motion"),"V50 needs responsive and reduced-motion contracts");
assert(css.includes(".droneLens")&&css.includes(".storyImage"),"V50 must apply the cinematic image-treatment system");
