...
const jsonRoute=readFileSync("app/api/recommend/route.ts","utf8"),streamRoute=readFileSync("app/api/recommend/stream/route.ts","utf8"),orchestrator=readFileSync("lib/ai/travel-orchestrator-v15.ts","utf8"),research=readFileSync("lib/ai/recommendation-research-agent-v14.ts","utf8"),ui=readFileSync("components/travel-decision-experience.tsx","utf8"),cityRoute=readFileSync("app/api/stay-cities/route.ts","utf8");
for(const [name,source] of [["json",jsonRoute],["stream",streamRoute]] as const){
 assert(source.includes("runTravelOrchestratorV15"),`${name} route must use the V15 orchestrator`);
...
console.log("V15_AGENT_ARCHITECTURE_OK",JSON.stringify({roles:ids,runtime:runtime.map(role=>role.id),development:development.map(role=>role.id),sharedOrchestrator:true,inventoryCitySelector:true,groundedScout:true}));
