...
const westernRequest:TripRequest={origin:"Athens",startDate:"2026-10-16",endDate:"2026-10-18",month:"october",nights:2,budget:800,moods:["relax","food"],travelerType:"friends",language:"el",distancePreference:"any",pace:"balanced",hotelStyle:"boutique",avoid:"none",entryMode:"unknown",groupSize:3,desiredEnergy:"balanced",socialPreference:"balanced",noveltyPreference:"balanced",mustHave:"none",dateFlexibility:"open",transportMode:"electric-car",stayLocationPreference:"balanced",tripText:"θέλω μόνο δυτική ελλάδα"};
const westernPre=preRankV8(westernRequest,structuredIntent(westernRequest),catalog,30),westernWeather=new Map(westernPre.map(item=>[item.destination.slug,evidence(item.destination.monthFit[9]??60,20)])),westernSelected=diversifyV8(finalRankV8(westernRequest,structuredIntent(westernRequest),westernPre,westernWeather),12),westernSlugs=new Set(["corfu","paxos","lefkada","kefalonia","zakynthos","ioannina","zagori","parga","nafpaktos","patras","olympia"]);
assert(westernSelected.length>=6,`screenshot profile should retain broad western choice, got ${westernSelected.length}`);checks+=1;
assert(westernSelected.every(item=>item.destination.regionGroup==="ionian"||item.destination.regionGroup==="epirus"||westernSlugs.has(item.destination.slug)),`western-only screenshot profile leaked ${westernSelected.map(item=>item.destination.slug)}`);checks+=1;
...
console.log("V10_100_SCENARIOS_OK",JSON.stringify({scenarios:100,checks,catalogSize:catalog.length,finalistCoverage,winnerCoverage,maxFinalistShare,maxWinnerShare,westernScreenshotProfile:westernSelected.map(item=>item.destination.slug),topFinalists:[...finalistFrequency.entries()].sort((a,b)=>b[1]-a[1]).slice(0,10),strictExplorer:strictSelected.map(item=>({slug:item.destination.slug,role:item.explorationRole})),examples:Object.fromEntries(Object.entries(examples).slice(0,5))}));
}
void main();
