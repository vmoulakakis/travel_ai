...
const gr=read("app/page.tsx"),en=read("app/en/page.tsx"),layout=read("app/layout.tsx"),v40=gr.includes("V40DiscoveryExperience"),home=read(v40?"components/v40-discovery-experience.tsx":"components/v36-escape-funnel.tsx"),css=read(v40?"components/v40-discovery-experience.module.css":"components/v36-escape-funnel.module.css"),ranker=read("lib/decision/solution-ranking-v36.ts"),route=read("app/api/escape/solve-v36/stream/route.ts");
const failures:string[]=[];
const expect=(condition:boolean,message:string)=>{if(!condition)failures.push(message)};

expect(v40?(gr.includes("V40DiscoveryExperience")&&en.includes("V40DiscoveryExperience")):(/V\d+EscapeFunnel/.test(gr)&&/V\d+EscapeFunnel/.test(en)),"current escape experience must own both home routes");
expect(v40?(home.includes("Travel Agent")&&home.includes("progress")&&home.includes("solutions=result?.solutions.slice(0,3)")):home.includes('type Stage="mood"|"dna"|"time"|"reality"|"solve"|"results"'),"active consumer surface must remain a bounded decision experience");
expect(home.includes('/api/escape/solve-v36/stream'),"active architecture must retain the bidirectional solve route");
expect(v40?(home.includes("real stay inventory")&&home.includes("3 προορισμοί")):(home.includes("Βρες 10 πραγματικές λύσεις")&&home.includes("Find 10 real solutions")),"active discovery surface must preserve real-inventory decision truth while presenting a bounded destination set");
expect(v40?(home.includes("destination")&&home.includes("stay")&&home.includes("inventory")):home.includes("Need → Destination → Stay ⇄ Stay Inventory → Destination → Need"),"bidirectional reasoning contract must remain represented in the active experience");
expect(v40?home.includes("mode=aerial"):(home.includes("setInterval")&&home.includes("6200")),"destination media must remain cinematic and sourced");
expect(home.includes("slice(0,3)")||home.includes("slice(0, 3)"),"active discovery must expose multiple bounded frames/options");
expect(v40?(css.includes("max-height:calc(100vh")||css.includes("min-height:calc(100vh"))):(css.includes("height:100dvh")&&css.includes("overflow:hidden")),"active shell must behave as a focused app instead of an endless questionnaire");
