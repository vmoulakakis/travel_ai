...
const activeSolver=v50Family?read("app/api/v50/agent/route.ts"):read("app/api/escape/solve-v42/route.ts");
...
expect(
  (isV54&&gr.includes("V54FinalHome")&&en.includes("V40DiscoveryExperience"))
  ||(isV50&&gr.includes("V50TravelIntelligenceHome")&&en.includes("V40DiscoveryExperience"))
  ||(!v50Family&&gr.includes("V40DiscoveryExperience")&&en.includes("V40DiscoveryExperience")),
  "Greek home may advance to V54/V50 while English remains on supported V40 until separately migrated"
);
expect(
  isV54
   ? home.includes("AI Travel Planner")&&home.includes("cards.slice")&&activeSolver.includes(".slice(0,10)")
   : isV50
...
expect(
  v50Family?home.includes('/api/v50/agent'):home.includes('/api/escape/solve-v42'),
...
expect(
  isV54?home.includes("heroMedia")&&css.includes(".hero"):isV50?home.includes("heroMedia")&&css.includes(".heroMedia"):home.includes("mode=aerial"),
  "destination media must remain cinematic and sourced"
);
expect(
  v50Family?activeSolver.includes(".slice(0,10)"):(home.includes("slice(0,3)")||home.includes("slice(0, 3)")),
  "active discovery must expose multiple bounded options"
);
expect(
  isV54?css.includes(".page")&&css.includes(".hero")&&css.includes("@media"):isV50?css.includes(".stage")&&css.includes("overflow:hidden"):css.includes("min-height:100svh")&&css.includes("overflow:hidden"),
...
expect(
  v50Family?activeSolver.includes("trackingUrl:offer.trackingUrl"):v42.includes("trackingUrl:x.row.tracking_url"),
...
console.log(`Bidirectional funnel compatibility smoke passed: active=${isV54?"V54 final AI site + V50 agent":isV50?"V50 persistent agent + Top 10 live stays":"V40/V42 semantic shell"}, legacy V36 fallback retained.`);
