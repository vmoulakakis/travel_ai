...
const parsed=structuredIntent(trip);assert.ok(parsed.weights.food>1);assert.ok((parsed.semantic?.negative.nightlife??0)>.5);
...
const strongStay=scoreGlobalStayCandidateV21(stay(true,boutiqueVector),trip,spec),weakStay=scoreGlobalStayCandidateV21(stay(false,genericVector),trip,spec);assert.ok(strongStay.eligible&&weakStay.eligible);assert.ok(strongStay.score>weakStay.score+10);
const hardSpec:StayConstraintSpec={hard:["BEACHFRONT"],soft:[],confidence:"HIGH",source:"deterministic",needsSemanticAssist:false};assert.equal(scoreGlobalStayCandidateV21(stay(false,genericVector),trip,hardSpec).eligible,false,"derived evidence must keep hard stay constraints fail-closed");
const rawPortfolio=diversifyV8([ranked(food,82),ranked(destination("relevant"),70),ranked(destination("weak"),45)],12,trip),portfolio=filterPortfolioV21(rawPortfolio);assert.ok(portfolio.some(item=>item.destination.slug==="relevant"));assert.ok(!portfolio.some(item=>item.destination.slug==="weak"));
const compromise=filterPortfolioV21([ranked(destination("c1"),55),ranked(destination("c2"),44),ranked(destination("c3"),38),ranked(destination("c4"),33)]);assert.equal(compromise.length,4);
