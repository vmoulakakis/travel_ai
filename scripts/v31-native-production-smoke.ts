...
const selectedExperiencePath=isV50?"components/v50-stay-funnel.tsx":isV40?"components/v40-stay-workspace.tsx":currentVersion==="v38"?"components/v38-escape-builder-client.tsx":"components/v34-escape-builder-client.tsx";
const solverPath=isV50?"app/api/v50/agent/route.ts":isV40?"app/api/escape/solve-v42/route.ts":currentVersion==="v34"?"app/api/escape/solve/stream/route.ts":"app/api/escape/solve-v36/stream/route.ts";
...
must(isV50?(solver.includes("offerScore")&&solver.includes("truthScore")&&solver.includes(".slice(0,10)")):isV40?(solver.includes("destinationScore")&&solver.includes("stayScore")&&solver.includes("valueScore")&&solver.includes("locationScore")):(ranking.includes("combinedScore")&&(ranking.includes("stayScore")||ranking.includes("inventoryScore"))),"Current solution ranking must combine destination and real stay/inventory evidence");
