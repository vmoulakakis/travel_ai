...
if(home.includes("V67FourStepFunnel")){const ui=read("components/v67-four-step-funnel.tsx"),selector=read("app/api/v67/select-one/route.ts");must(ui.includes("/api/v67/select-one")&&ui.includes("/api/trip-builder")&&ui.includes("/api/guide/email"),"V67 funnel wiring missing");must(selector.includes("getDailyTripWeatherV25")&&selector.includes("demandIsDiscriminating"),"V67 intelligence selection missing");console.log("Current V67 four-step experience + V31 support routes smoke: OK");process.exit(0)}
...
must(isV54?(escapeFunnel.includes("cards.slice")&&escapeFunnel.includes("v65StarAi")&&escapeFunnel.includes("v65StarDemand")&&escapeFunnel.includes("mapIntelligence")&&escapeFunnel.includes("flyTo")&&escapeFunnel.includes("runAgent")):isV50?(escapeFunnel.includes("solutions.map")&&escapeFunnel.includes("v50Pin")&&escapeFunnel.includes("Travel DNA")&&escapeFunnel.includes("flyTo")&&escapeFunnel.includes("callAgent")):isV40?cinematicDestinationFirst:(escapeFunnel.includes("resultRail")&&escapeFunnel.includes("solutions.map")&&escapeFunnel.includes("combinedScore")),"Current discovery experience must expose real ranked destinations before stay selection");
