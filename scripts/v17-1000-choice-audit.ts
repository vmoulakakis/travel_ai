...
async function main(){
 ...
 for(let index=0;index<CASES;index++){
  ...
  if(request.consideredDestination){const target=exactDestination(request.consideredDestination,catalog);if(target){ideaEligible++;const pos=result.findIndex(item=>item.destination.slug===target.slug);if(pos>=0)ideaVisible++;if(pos>=0&&pos<3)ideaTop3++;}}
