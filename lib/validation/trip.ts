...
export interface TripRequest {
  ...
  stayLocationPreference?:StayLocationPreference;
...
const value=input as Record<string,unknown>,errors:string[]=[];
  ...
  if(!entryModes.has(entryMode))errors.push("invalid entryMode");if(!Number.isInteger(groupSize)||groupSize<1||groupSize>10)errors.push("groupSize must be between 1 and 10");if(!energies.has(desiredEnergy))errors.push("invalid desiredEnergy");if(!socialPreferences.has(socialPreference))errors.push("invalid socialPreference");if(!noveltyPreferences.has(noveltyPreference))errors.push("invalid noveltyPreference");if(!mustHaves.has(mustHave))errors.push("invalid mustHave");if(!dateFlexibilities.has(dateFlexibility))errors.push("invalid dateFlexibility");if(!transportModes.has(transportMode))errors.push("invalid transportMode");if(!stayLocationPreferences.has(stayLocationPreference))errors.push("invalid stayLocationPreference");if(consideredDestination.length>80)errors.push("consideredDestination must be <= 80 characters");
  if(errors.length)return{success:false,errors};
  return{success:true,data:{origin,startDate,endDate,month,nights,budget,moods:parsedMoods,travelerType,language,distancePreference,pace,hotelStyle,avoid,entryMode,groupSize,desiredEnergy,socialPreference,noveltyPreference,mustHave,dateFlexibility,transportMode,stayLocationPreference,...(consideredDestination?{consideredDestination}:{}),...(refinement?{refinement}:{}),...(tripText?{tripText}:{})}};
}
