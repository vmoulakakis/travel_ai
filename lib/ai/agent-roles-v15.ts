...
export const TRAVEL_AGENT_ROLES_V15:readonly TravelAgentRoleV15[]=[
 {id:"orchestrator",titleEl:"Αρχιτέκτονας Απόφασης",titleEn:"Decision Orchestrator",scope:"runtime",kind:"deterministic",mission:"Run one authoritative decision path, enforce stage order, budgets and safe fallbacks.",allowedTools:["intent","catalog","location truth","weather","research","verifier","auditor","council","learning"],forbidden:["invent facts","override hard constraints","rank by affiliate economics"],success:"JSON and streaming APIs execute identical decision logic."},
 {id:"location-truth",titleEl:"Ελεγκτής Τοποθεσίας",titleEn:"Location Truth Agent",scope:"runtime",kind:"deterministic",mission:"Resolve selected destinations, natural-language cities, Greek inflections, regions and country scope into one canonical candidate universe before any ranking or LLM judgement.",allowedTools:["canonical destination catalog","location aliases","region rules","country scope guard"],forbidden:["downgrade a selected destination to a score bonus","replace an unsupported foreign city with a Greek destination","let a downstream agent add an out-of-scope slug"],success:"Zero destination/country scope leaks from input to final recommendations."},
 {id:"intent-constraint",titleEl:"Μεταφραστής Πρόθεσης",titleEn:"Intent & Constraint Interpreter",scope:"runtime",kind:"hybrid",mission:"Convert structured answers and optional free text into preference weights while deterministic guards own explicit constraints.",allowedTools:["structured intent","optional DeepSeek/self-hosted parser","location truth guard"],forbidden:["name destinations","invent routes","soften explicit exclusions"],success:"User meaning changes fit without creating facts."},
 ...
 {id:"regression-judge",titleEl:"Regression Judge",titleEn:"Regression Judge",scope:"development",kind:"deterministic",mission:"Run population, adversarial, city-inventory, location-scope and agent-tool invariants before merge.",allowedTools:["1000-search audit","10000 constraint audit","location truth audit","map ranking smoke tests"],forbidden:["grade exact prose","accept nondeterministic hard-constraint leaks"],success:"Zero hard-constraint leaks, zero location leaks and zero unsupported city options."},
] as const;
