...
import { runLocalityEvidenceRerankerV23 } from "@/lib/ai/locality-evidence-reranker-v23";
...
export async function runTravelOrchestratorV23(trip:TripRequest,sessionId:string,emit:TravelOrchestratorEmitter=noop):Promise<V8RecommendationResponse>{
 ...
 try{
  ...
  const rawPre=preRankV8(rankingTrip,intent,constrainedCatalog,Math.max(30,constrainedCatalog.length)),semanticPre=applySemanticIntentRankingV18(rawPre,intent),criterionPre=applyCriterionRelevanceV22(trip,intent,semanticPre,choiceProfiles),localityPre=applyLocalityFuzzyRankingV23(trip,intent,criterionPre.ranked,localities);stage="locality-evidence";
  const localityEvidence=await runLocalityEvidenceRerankerV23(trip,intent,localityPre.ranked,localities,llmBudget);mark("locality-evidence");signal("locality:evidence",38,{agent:"locality-evidence-reranker",source:localityEvidence.audit.source,inspected:localityEvidence.audit.inspectedLocalities.length,model:localityEvidence.audit.model});stage="choice-correctness";
  const choice=await applyChoiceCorrectnessV21(trip,intent,stayRequirements,localityEvidence.ranked),preAll=choice.ranked.slice(0,30),minimum=(hardConstraint||stayRequirements.hard.length)?1:3;mark("pre-rank");
  signal("choice:ready",43,{agent:"fuzzy-matcher",localities:localityPre.audit.localityCount,mappedDestinations:localityPre.audit.mappedDestinations,freeTextWeight:localityPre.audit.freeTextWeight,localityEvidence:localityEvidence.audit.source,globalStayScan:choice.audit.stayScanRan&&!choice.audit.stayScanFailed,mappedStays:choice.audit.mappedStayCount,hardStayRejected:choice.audit.hardStayRejected.length});
