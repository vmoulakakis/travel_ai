...
export function structuredIntent(request:TripRequest):V8IntentProfile{
 const w=blank();for(const mood of request.moods){const map:Record<string,V8Dimension>={romantic:"romantic",relax:"relax",food:"food",culture:"culture",city:"city",nature:"nature",adventure:"adventure",warmth:"warmth"};const d=map[mood];if(d)w[d]=1;}
 ...
 if(request.hotelStyle==="luxury")w.luxury=Math.max(w.luxury,.3);
 if(request.hotelStyle==="value"||request.avoid==="high-cost")w.value=Math.max(w.value,.4);
 if(request.hotelStyle==="resort"){w.relax=Math.max(w.relax,.35);w.wellness=Math.max(w.wellness,.25);}
 if(request.hotelStyle==="boutique")w.romantic=Math.max(w.romantic,.25);
...
export async function interpretIntentV8(request:TripRequest):Promise<V8IntentProfile>{
 ...
 try{
  ...
  // Explicit answers always remain dominant.
