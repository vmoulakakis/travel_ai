...
export async function runTravelOrchestratorV15(trip:TripRequest,sessionId:string,emit:TravelOrchestratorEmitter=noop):Promise<V8RecommendationResponse>{
 ...
 try{
  ...
  // V21: interpretation is still canonical and single-pass, but candidate generation now enforces strong semantic exclusions
  // and performs one global dated stay scan before finalists are frozen. Inventory COUNT never adds rank; only best property fit can.
  const rawPre=preRankV8(rankingTrip,intent,constrainedCatalog,Math.max(30,constrainedCatalog.length));
  const semanticPre=applySemanticIntentRankingV18(rawPre,intent);
  stage="choice-correctness";
  const choice=await applyChoiceCorrectnessV21(trip,intent,stayRequirements,semanticPre),preAll=choice.ranked.slice(0,30),minimum=(hardConstraint||stayRequirements.hard.length)?1:3;mark("pre-rank");
  signal("choice:ready",39,{agent:"orchestrator",semanticRejected:choice.audit.semanticRejected.length,globalStayScan:choice.audit.stayScanRan&&!choice.audit.stayScanFailed,mappedStays:choice.audit.mappedStayCount,hardStayRejected:choice.audit.hardStayRejected.length});
  if(preAll.length<minimum){
   writeRecommendationAudit({sessionId,status:"no-result",stage:"pre-rank",timingsMs:{...timings,total:Date.now()-started},intentSource:intent.source,hardConstraint:hardConstraint?.id??null,stayRequirements:stayRequirementAudit(stayRequirements,{choiceCorrectness:choice.audit}),llmBudget:llmBudget.snapshot(),catalogSize:catalog.length,preCandidates:preAll.map(item=>item.destination.slug),auditor:{roles}});
