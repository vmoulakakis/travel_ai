...
function mapOffer(row: ApiOffer): AffiliateOffer | null {
  ...
  return {
    ...
    currency: text(row.currency), price: num(row.price), fullPrice: num(row.full_price), discount: num(row.discount), demandSignal: num(row.demand_proxy),
...
function mapCandidate(row: ApiCandidate): AffiliateDestinationCandidate | null {
  ...
  return {
    ...
    propertyCount: Number(row.property_count ?? 0), activeOfferCount: Number(row.active_offer_count ?? 0), minPrice: num(row.min_price), medianPrice: num(row.median_price),
    maxPrice: num(row.max_price), currency: text(row.currency), demandScore: Number(row.demand_score ?? 0), saleOfferCount: Number(row.sale_offer_count ?? 0),
