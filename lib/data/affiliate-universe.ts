...
const monthMap: Record<Exclude<TripRequest["month"], "flexible">, string> = { september: "09", october: "10", november: "11" };
...
function mapOffer(row: ApiOffer): AffiliateOffer | null {
  ...
  return {
    sourceProductId, propertyName, description: text(row.description), category: text(row.source_category), programId: text(row.program_id), trackingUrl,
...
function mapCandidate(row: ApiCandidate): AffiliateDestinationCandidate | null {
  const destinationId = text(row.destination_id); const locationLabel = text(row.location_label);
  ...
  return {
    destinationId, locationLabel, countryHint: text(row.country_hint), latitude: num(row.centroid_latitude), longitude: num(row.centroid_longitude),
...
export function monthParam(month: TripRequest["month"]): string {
  if (month === "flexible") return "flexible";
  return `${new Date().getUTCFullYear()}-${monthMap[month]}`;
}
...
export async function loadAffiliateUniverse(request: TripRequest, limit = 100): Promise<AffiliateDestinationCandidate[]> {
  ...
  const url = `${base}?month=${encodeURIComponent(monthParam(request.month))}&limit=${Math.max(15, Math.min(limit, 160))}`;
  const controller = new AbortController(); const timer = setTimeout(() => controller.abort(), 7000);
  try {
    const response = await fetch(url, { cache: "no-store", signal: controller.signal, headers: { "user-agent": "travel-ai-guru/2.0" } });
...
export async function loadAffiliateDestinationDetail(destinationId: string, month: TripRequest["month"]): Promise<AffiliateDestinationDetailResponse> {
  ...
  const url = `${base}?destination_id=${encodeURIComponent(destinationId)}&month=${encodeURIComponent(monthParam(month))}`;
  const controller = new AbortController(); const timer = setTimeout(() => controller.abort(), 7000);
  try {
    const response = await fetch(url, { cache: "no-store", signal: controller.signal, headers: { "user-agent": "travel-ai-guru/2.0" } });
    if (!response.ok) throw new Error(`Destination detail ${response.status}`);
    const row = await response.json() as ApiDetail;
    ...
    return {
      version: Number(row.version ?? 1), source: "linkwise-json-only", range: text(row.range) ?? monthParam(month), generatedAt: text(row.generatedAt) ?? new Date().toISOString(),
      ...
      destination: row.destination && typeof row.destination === "object" ? row.destination as AffiliateDestinationDetailResponse["destination"] : null,
      premiumOffers, premiumFill, alternatives
    };
  } finally { clearTimeout(timer); }
}
...
export async function loadFeaturedAffiliateDestinations(limit = 5): Promise<Array<{ destinationId: string; locationLabel: string; imageUrl: string }>> {
  const request: TripRequest = { origin: "Athens", month: "flexible", nights: 3, budget: 500, moods: ["relax"], travelerType: "couple", language: "el", distancePreference: "any", pace: "balanced", hotelStyle: "any", avoid: "none" };
