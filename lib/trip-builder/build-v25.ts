...
import type { FlexibleWindowV25,TripBuilderPlanV25,TripBuilderRequestV25 } from "@/lib/trip-builder/types-v25";
...
export function sameWeekdayFlexibleWindowsV25(startDate:string,endDate:string){
 const nights=Math.round((Date.parse(`${endDate}T00:00:00Z`)-Date.parse(`${startDate}T00:00:00Z`))/DAY),out:Array<{startDate:string;endDate:string;shiftDays:number}>=[];
 for(const shiftDays of[-14,-7,7,14,21,28]){const start=addDays(startDate,shiftDays);if(start<today())continue;out.push({startDate:start,endDate:addDays(start,nights),shiftDays});}
...
function guidePath(slug:string,offerId:string,startDate:string,endDate:string){const p=new URLSearchParams({slug,offer:offerId,start:startDate,end:endDate});return`/travel-guide?${p}`}
...
export async function buildTripBuilderV25(input:TripBuilderRequestV25):Promise<TripBuilderPlanV25>{
 ...
 const groupSize=Math.max(1,trip.groupSize??1),lat=selected.latitude??destination.latitude,lon=selected.longitude??destination.longitude,summer=isSummer(trip.startDate,trip.endDate);
 const[availability,weather,tripadvisor]=await Promise.all([
  ...
  getTripadvisorBundleV25({destinationName:trip.language==="en"?destination.nameEn:destination.nameEl,hotelName:selected.propertyName,latitude:lat,longitude:lon,isSummer:summer,language:trip.language==="en"?"en":"el"}),
 ]);
