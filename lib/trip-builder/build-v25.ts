...
function mapWebResearch(result:Awaited<ReturnType<typeof researchDestination>>):InternetResearchV25{return{status:result.source==="verified-synthesis"?"verified-synthesis":result.source==="research-pending"?"research-pending":"unavailable",overview:result.overview,bullets:result.attractions.slice(0,5).map(item=>({name:item.name,summary:item.summary,why:item.whyItFits??null,strength:item.evidenceStrength})),practicalNotes:result.practicalNotes.slice(0,5),sources:result.sources.map(source=>({title:source.title,url:source.url,domain:source.domain}))}}
