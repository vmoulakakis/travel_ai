...
function guidePath(slug:string,offerId:string,trip:TripRequest){const tripToken=Buffer.from(JSON.stringify(trip),"utf8").toString("base64url"),p=new URLSearchParams({slug,start:trip.startDate,end:trip.endDate,offer:offerId,trip:tripToken});return`/api/guide?${p}`}
...
export async function buildTripBuilderV25(input:TripBuilderRequestV25):Promise<TripBuilderPlanV25>{
 ...
 const candidates=sameWeekdayFlexibleWindowsV25(trip.startDate,trip.endDate),flexibleWindows:FlexibleWindowV25[]=await Promise.all(candidates.map(async candidate=>{const shiftedTrip=withDates(trip,candidate.startDate,candidate.endDate),[availabilityResult,windowWeather]=await Promise.all([assessStayWindowV25({slug,startDate:candidate.startDate,endDate:candidate.endDate,offerId:selected.sourceProductId,propertyName:selected.propertyName,groupSize,probeProvider:true}),getDailyTripWeatherV25(shiftedTrip,lat,lon)]);return{startDate:candidate.startDate,endDate:candidate.endDate,shiftDays:candidate.shiftDays,sameWeekdayPattern:true as const,weatherScore:windowWeather.score,weatherLabel:windowWeather.label,availability:availabilityResult.truth,alternativeStayCount:availabilityResult.alternativeStayCount}}));
