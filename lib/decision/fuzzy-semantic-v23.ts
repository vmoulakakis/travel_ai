...
const lowEffortPattern=/(?:\b(?:easy access|easy transfer|little driving|low driving|nearby|close|without much driving|avoid long drives?|not a long transfer|xoris poli odigisi|ligi odigisi|ligo odigisi|eukoli metavasi|na min xasoume ores sto dromo|xoris megali diadromi)\b|ευκολ.{0,12}(?:προσβ|μεταβ)|λιγ.{0,12}οδηγ|χωρις.{0,18}(?:πολλη|πολυ).{0,8}οδηγ|να μη.{0,20}(?:ωρες|μερα).{0,20}δρομ|κοντιν)/i;
...
function applyExtraFreeText(text:string,positive:Partial<Record<V23SemanticDimension,number>>,negative:Partial<Record<V23SemanticDimension,number>>){
 ...
 for(const [dimension,pattern] of Object.entries(terms) as Array<[V23SemanticDimension,RegExp]>){
  for(const clause of clauses){pattern.lastIndex=0;if(!pattern.test(clause))continue;
   // Low-effort language commonly contains surface negation ("without driving", "xoris poli odigisi").
   // The semantic target is positive LOW_EFFORT, so do not invert it.
   if(dimension==="low_effort"){maxSet(positive,dimension,.82);continue;}
   const negated=/(?:\b(?:no|not|without|avoid|exclude)\b|(?:οχι|χωρις|αποφυγ)|\b(?:xoris|oxi)\b)/i.test(clause);maxSet(negated?negative:positive,dimension,negated ? .9 : .72);
  }
 }
}

export function buildFuzzyContractV23(request:TripRequest,legacy?:V8SemanticIntent):V23FuzzyIntentContract{
 const positive:Partial<Record<V23SemanticDimension,number>>={},negative:Partial<Record<V23SemanticDimension,number>>={};applyStructured(request,positive);applyLegacySemantic(positive,negative,legacy);if(request.tripText)applyExtraFreeText(request.tripText,positive,negative);
 ...
 if(qualifiers.easyAccess>0)maxSet(positive,"low_effort",.92*qualifiers.easyAccess);if(qualifiers.walkable>0){maxSet(positive,"city",.46*qualifiers.walkable);maxSet(positive,"low_effort",.32*qualifiers.walkable);}if(qualifiers.localCharacter>0)maxSet(positive,"culture",.72*qualifiers.localCharacter);
...
export function mergeFuzzyContractV23(base:V23FuzzyIntentContract,parsed:{positive?:Partial<Record<V23SemanticDimension,number>>;negative?:Partial<Record<V23SemanticDimension,number>>;priorities?:V23SemanticDimension[];qualifiers?:Partial<V23FuzzyIntentContract["qualifiers"]>;confidence?:number},source:V23FuzzyIntentContract["source"]):V23FuzzyIntentContract{
 const positive={...base.positive},negative={...base.negative};for(const [d,v] of Object.entries(parsed.positive??{}) as Array<[V23SemanticDimension,number]>){if(v>0)maxSet(positive,d,v);}for(const [d,v] of Object.entries(parsed.negative??{}) as Array<[V23SemanticDimension,number]>){if(v>0)maxSet(negative,d,v);}for(const d of V23_SEMANTIC_DIMENSIONS){if((negative[d]??0)>=.78&&(positive[d]??0)>.45)positive[d]=.45;}
 const priorities=parsed.priorities?.length?[...new Set(parsed.priorities)]:base.priorities,qualifiers={...base.qualifiers,...parsed.qualifiers};if(qualifiers.easyAccess>0)maxSet(positive,"low_effort",.95*qualifiers.easyAccess);if(qualifiers.localCharacter>0)maxSet(positive,"culture",.76*qualifiers.localCharacter);if(qualifiers.walkable>0)maxSet(positive,"city",.5*qualifiers.walkable);
...
export function fuzzyVectorFitV23(candidate:number[],contract:V23FuzzyIntentContract){
 if(candidate.length!==24)return{fit:.5,weighted:.5,priorityFloor:.5,negativeConflict:0,activeCriteria:0};let sum=0,weights=0,negativeConflict=0,negativeWeights=0;const prioritySet=new Map(contract.priorities.slice(0,4).map((d,i)=>[d,[2.15,1.65,1.35,1.18][i]]));
 ...
 const weighted=weights?sum/weights:.5,priorityValues=contract.priorities.slice(0,3).filter(d=>(contract.positive[d]??0)>=.3).map(d=>clamp01(Number(candidate[idx[d]]??.5))),priorityFloor=priorityValues.length?Math.min(...priorityValues):weighted,fit=clamp01(weighted*.74+priorityFloor*.26);return{fit,weighted,priorityFloor,negativeConflict:negativeWeights?negativeConflict/negativeWeights:0,activeCriteria:weights};
}
