...
// keep the feasible breadth and let public fitStatus/confidence make that trade-off explicit.
export function filterPortfolioV21(items:readonly V8Ranked[]):V8Ranked[]{
  ...
  const floor=Math.max(50,best-16),kept=items.filter(item=>item.score>=floor&&item.breakdown.season>=40&&item.breakdown.budget>25&&item.breakdown.crowdFit>25);
  return kept.length?kept:[items[0]];
}
