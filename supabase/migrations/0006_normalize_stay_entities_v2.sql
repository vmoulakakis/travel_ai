create or replace function public.normalize_stay_property_name(input text)
returns text
language sql
immutable
as $$
  select trim(regexp_replace(regexp_replace(split_part(coalesce(input,''),'✦',1), '<[^>]+>', ' ', 'g'), '\s+', ' ', 'g'));
$$;
...
begin
  ...
  begin
    ...
    select x,
      ...
      public.normalize_stay_property_name(split_part(coalesce(x->>'product_name',''),'|',1)) as property_name,
      coalesce(nullif(trim(split_part(coalesce(x->>'product_name',''),'|',2)),''), nullif(x->>'address','')) as location_label,
      nullif(regexp_replace(coalesce(x->>'address',''), '<[^>]+>', ' ', 'g'),'') as address,
      nullif(x->>'city','') as city_raw,
      nullif(x->>'category','') as category,
      nullif(regexp_replace(coalesce(x->>'description',''), '<[^>]+>', ' ', 'g'),'') as description,
    ...
    where nullif(x->>'latitude','') is not null
      ...
      and nullif(public.normalize_stay_property_name(split_part(coalesce(x->>'product_name',''),'|',1)),'') is not null;
    ...
    delete from public.stay_offers o
    where not exists (select 1 from tmp_stay_feed f where f.product_id=o.source_product_id);

    delete from public.stay_places p
    where not exists (select 1 from public.stay_offers o where o.place_id=p.id);
    ...
    delete from public.destination_supply_signals s
    where not exists (select 1 from tmp_stay_feed f where f.location_label=s.location_label);
    ...
    update public.feed_ingestion_runs set
      ...
      diagnostics=jsonb_build_object('normalization','property-v2','stale_cleanup',true)
    where id=v_run;
...
revoke all on function public.normalize_stay_property_name(text) from public, anon, authenticated;
grant execute on function public.normalize_stay_property_name(text) to postgres, service_role;
