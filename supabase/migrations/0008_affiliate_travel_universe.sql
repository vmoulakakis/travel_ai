...
as $$
with eligible as (
  select o.* from public.stay_offers o
  where o.tracking_url is not null and o.tracking_url <> ''
  ...
  select md5(lower(e.location_label)) as destination_id,e.location_label,
    ...
    min(e.price) as min_price,percentile_cont(0.5) within group(order by e.price) filter(where e.price is not null) as median_price,max(e.price) as max_price,
    ...
    coalesce(sum(e.demand_proxy),0) as demand_score,count(*) filter(where e.on_sale is true or coalesce(e.discount,0)>0) as sale_offer_count,
    max(e.discount) as max_discount,avg(e.discount) filter(where e.discount is not null) as avg_discount,
    ...
    max(e.valid_to) as valid_to_max,left(string_agg(distinct concat_ws(' ',e.property_name,e.description,e.source_category),' · '),6000) as semantic_text
  from eligible e left join public.stay_places p on p.id=e.place_id
  where e.location_label is not null and trim(e.location_label)<>'' group by e.location_label
), ranked as (
  select g.*,
    (select jsonb_agg(to_jsonb(x) order by x.demand_proxy desc nulls last,x.discount desc nulls last,x.price asc nulls last)
     from (select e.source_product_id,e.place_id,e.property_name,e.description,e.source_category,e.program_id,e.tracking_url,e.image_url,e.thumb_url,e.availability,e.valid_from,e.valid_to,e.on_sale,e.currency,e.price,e.full_price,e.discount,e.demand_proxy,e.raw->>'model_name' as model_name,e.raw->>'brand_name' as brand_name,e.raw->'custom' as custom,e.raw->'extra_images' as extra_images,e.raw->'variations' as variations from eligible e where e.location_label=g.location_label order by coalesce(e.demand_proxy,0) desc,coalesce(e.discount,0) desc,e.price asc nulls last limit 5) x) as top_offers
  from grouped g
)
select r.* from ranked r order by r.active_offer_count desc,r.demand_score desc,r.min_price asc nulls last limit greatest(3,least(coalesce(p_limit,120),250));
$$;
