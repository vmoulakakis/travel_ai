...
Deno.serve(async(req:Request)=>{
  ...
  const truth=await response.json() as TruthPayload;
  const destinations=Math.max(0,Number(truth.activeGreekDestinations)||0),covered=Math.max(0,Number(truth.verifiedEvidenceDestinations)||0);
  const evidenceCoveragePercent=destinations?Number(((covered/destinations)*100).toFixed(2)):0;
  const unknownStockOffers=Math.max(0,Number(truth.unknownStockOffers)||0),confirmedStockOffers=Math.max(0,Number(truth.confirmedStockOffers)||0);

  return json({
    version:20,
    release:"V20",
    source:"production-truth-v20",
    checkedAt:truth.checkedAt??new Date().toISOString(),
    evidenceCoveragePercent,
    availabilitySemantics:unknownStockOffers>0?"TRI_STATE_STOCK_TRUTH":"EXPLICIT_STOCK_TRUTH",
    evidenceDepth:evidenceCoveragePercent>=80?"BROAD":evidenceCoveragePercent>=30?"PARTIAL":"LIMITED",
    truth:{...truth,confirmedStockOffers,unknownStockOffers},
  });
});
