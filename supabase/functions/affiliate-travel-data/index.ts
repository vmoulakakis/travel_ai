...
Deno.serve(async(req:Request)=>{
  ...
  const url=new URL(req.url); const range=bounds(url.searchParams.get("month"));
  const limit=Math.max(12,Math.min(Number(url.searchParams.get("limit")||80),120));
  const rpc=await fetch(`${base}/rest/v1/rpc/get_affiliate_travel_candidates`,{method:"POST",headers:{apikey:key,Authorization:`Bearer ${key}`,"content-type":"application/json"},body:JSON.stringify({p_start:range.start,p_end:range.end,p_limit:limit})});
  if(!rpc.ok) return json({error:"Affiliate universe unavailable",detail:await rpc.text()},502);
  const candidates=await rpc.json();
  return json({version:2,source:"linkwise-json-only",range:range.value,generatedAt:new Date().toISOString(),candidateCount:Array.isArray(candidates)?candidates.length:0,candidates});
});
