...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const languagePair = (el: string, en: string) => ({
    languages: { "el-GR": el, "en-GB": en, "x-default": el },
  });
  const pair = (
    elPath: string,
    enPath: string,
    ...
    const el = `${base}${elPath}`;
    const en = `${base}${enPath}`;
    const alternates = languagePair(el, en);
    return [
      { url: el, changeFrequency: frequency, priority, alternates },
      { url: en, changeFrequency: frequency, priority, alternates },
    ];
  };
  ...
  return [
    {
      ...
      priority: 1,
      alternates: languagePair(base, `${base}/en`),
    },
    {
      url: `${base}/en`,
      changeFrequency: "weekly",
