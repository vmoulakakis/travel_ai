...
export default async function EscapePage({params,searchParams}:Props){
 ...
 const loaded=(await loadV8StayOffers(slug,start,end,18).catch(()=>[])).filter(offer=>offer.trackingUrl.startsWith("https://go.linkwi.se/")&&offer.trackingUrl.includes("/CD104/"));
 const offers=preferredOffer?[...loaded].sort((a,b)=>Number(b.sourceProductId===preferredOffer)-Number(a.sourceProductId===preferredOffer)).slice(0,10):loaded.slice(0,10);
