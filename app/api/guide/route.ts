...
export async function GET(request: Request) {
  ...
  const qrData = await QRCode.toDataURL(offer.trackingUrl, { errorCorrectionLevel: "H", margin: 1, width: 260, color: { dark: "#061424", light: "#FFFDF8" } });
  const qr = await pdf.embedPng(Buffer.from(qrData.split(",")[1], "base64"));
  const photo = await embedPhoto(pdf, offer.imageUrl || offer.thumbUrl);
  ...
  const footer = (page: PDFPage, pageNo: number) => {
    page.drawLine({ start: { x: 42, y: 76 }, end: { x: 553, y: 76 }, thickness: .7, color: rgb(.78, .8, .8) });
    page.drawImage(qr, { x: 45, y: 18, width: 47, height: 47 });
    page.drawText("ΣΚΑΝΑΡΕ ΓΙΑ ΤΗΝ ΤΕΛΙΚΗ ΤΙΜΗ ΚΑΙ ΔΙΑΘΕΣΙΜΟΤΗΤΑ", { x: 104, y: 55, size: 6.5, font: bold, color: palette.violet });
    const urlLines = wrap(offer.trackingUrl, regular, 4.8, 405).slice(0, 2);
    urlLines.forEach((line, index) => page.drawText(line, { x: 104, y: 42 - index * 7, size: 4.8, font: regular, color: palette.muted }));
    page.drawText(`${pageNo}/8`, { x: 525, y: 28, size: 7, font: bold, color: palette.muted });
  };
  ...
  cover.drawText("Ο ΠΡΟΣΩΠΙΚΟΣ ΣΟΥ ΟΔΗΓΟΣ · 8 ΣΕΛΙΔΕΣ", { x: 42, y: 792, size: 8, font: bold, color: palette.cyan });
  ...
  drawTextBlock(fit, "Η σωστή ερώτηση δεν είναι αν ο προορισμός είναι όμορφος. Είναι αν υποστηρίζει το ταξίδι που χρειάζεσαι τώρα.", bold, 17, 46, y - 20, 495, palette.violet, 24); footer(fit, 2);

  const rhythm = pdf.addPage(A4); drawTitle(rhythm, "02 · Ο ΡΥΘΜΟΣ", "Τέσσερις ημέρες χωρίς πρόγραμμα-μαραθώνιο", regular, bold);
  const days = [
  ...
  y = 650; for (const [label, copy] of days) { rhythm.drawText(label, { x: 48, y, size: 11, font: bold, color: palette.violet }); y = drawTextBlock(rhythm, copy, regular, 12, 145, y, 390, palette.ink, 18) - 24; } footer(rhythm, 3);

  const budget = pdf.addPage(A4); drawTitle(budget, "03 · ΤΟ BUDGET", "Πού αξίζει να δώσεις και πού όχι", regular, bold);
  ...
  ]; for (const [label, copy] of budgetItems) { budget.drawRectangle({ x: 44, y: y - 42, width: 507, height: 72, color: palette.white, borderColor: rgb(.82, .82, .82), borderWidth: .5 }); budget.drawText(label, { x: 59, y: y + 8, size: 11, font: bold, color: palette.violet }); drawTextBlock(budget, copy, regular, 10, 59, y - 12, 470, palette.ink, 14); y -= 91; } footer(budget, 4);

  const experiences = pdf.addPage(A4); drawTitle(experiences, "04 · Ο ΤΟΠΟΣ", "Εμπειρίες που υπηρετούν τον λόγο του ταξιδιού", regular, bold);
  ...
  ]; y = 650; experienceList.forEach((item, index) => { experiences.drawText(`0${index + 1}`, { x: 48, y, size: 27, font: bold, color: palette.cyan }); y = drawTextBlock(experiences, item, regular, 12, 110, y + 4, 425, palette.ink, 18) - 30; }); footer(experiences, 5);

  const risks = pdf.addPage(A4); drawTitle(risks, "05 · Ο ΕΙΛΙΚΡΙΝΗΣ ΕΛΕΓΧΟΣ", "Τι μπορεί να χαλάσει την επιλογή", regular, bold);
  ...
  drawTextBlock(final, "Αν κάτι δεν συμφωνεί με τις ημερομηνίες ή τις ανάγκες σου, γύρισε πίσω. Μία σωστή ταξιδιωτική απόφαση αντέχει και στον τελευταίο έλεγχο.", bold, 15, 46, 330, 500, palette.ink, 22); footer(final, 8);
