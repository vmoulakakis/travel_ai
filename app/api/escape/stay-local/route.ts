...
export async function POST(request:Request){
 try{
  ...
  const trip=parsed.data,[catalog,offers]=await Promise.all([loadV8DestinationCatalog(),loadV8StayOffers(slug,trip.startDate,trip.endDate,60)]),destination=catalog.find(x=>x.slug===slug),stay=offers.find(x=>x.sourceProductId===offerId);if(!destination||!stay)return NextResponse.json({message:"Stay not found"},{status:404});
