...
export async function POST(request:Request){
  ...
  const d=parsed.data;
  const response=await fetch(new URL("/rest/v1/travel_missions",base),{method:"POST",cache:"no-store",headers:{apikey:key,Authorization:`Bearer ${key}`,"Content-Type":"application/json",Prefer:"return=representation"},body:JSON.stringify({locale:d.locale,status:"profiled",origin_text:d.originText||null,travel_window:d.travelWindow,travelers:{type:d.travelerType,groupSize:d.groupSize},budget_eur:d.budgetEur??null,need_text:d.needText,escape_dna:{signals:d.dna}})});
