...
export async function POST(request:Request){
 ...
 const research=await fetch(new URL("/rest/v1/travel_research_runs",base),{method:"POST",cache:"no-store",headers:{...h,Prefer:"resolution=ignore-duplicates,return=minimal"},body:JSON.stringify({mission_id:parsed.data.missionId,destination_id:parsed.data.destinationId,status:"queued"})});
