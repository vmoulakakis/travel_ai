...
const Body=z.object({
 ...
 finalists:z.array(z.object({slug:z.string().min(1).max(120),score:z.number().min(0).max(100),why:z.string().max(1000),seasonNote:z.string().max(500).optional(),budgetLabel:z.string().max(300).optional(),effortLabel:z.string().max(300).optional()})).min(1).max(3)
});
