...
export async function GET(request:Request,{params}:{params:Promise<{slug:string}>}){
 try{
  ...
  return NextResponse.json({version:30,destination:{slug:destination.slug,nameEl:destination.nameEl,nameEn:destination.nameEn,latitude:destination.latitude,longitude:destination.longitude,regionGroup:destination.regionGroup,tags:destination.tags,monthFit:destination.monthFit,costTier:destination.costTier,crowdLevel:destination.crowdLevel,routeConfidence:destination.routeConfidence},inventory,tripadvisorSignal,tripadvisor,booking,stayWindow:{startDate,endDate,offers:trustedOffers},trustPolicy:"Ratings, review counts and rankings are displayed only when returned by their source APIs. Missing data is shown as unavailable, never estimated."},{headers:{"cache-control":"public, max-age=300, s-maxage=21600, stale-while-revalidate=43200","x-content-type-options":"nosniff"}});
 }catch{return NextResponse.json({error:"Destination evidence temporarily unavailable"},{status:503,headers:{"cache-control":"no-store"}})}
}
