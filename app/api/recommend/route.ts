...
export async function POST(request:Request){
 ...
 try{
  ...
  const weather=await enrichV8Weather(trip,pre.map(x=>x.destination),14),ranked=finalRankV8(trip,intent,pre,weather),selected=diversifyV8(ranked,6),selectedIds=new Set(selected.map(x=>x.destination.slug)),verifyPool=[...selected,...ranked.filter(x=>!selectedIds.has(x.destination.slug))].slice(0,10),verification=await verifyV8(trip,verifyPool),fixed=verification.checked&&!verification.passed?repair(selected,ranked,verification.rejectSlugs):selected;
