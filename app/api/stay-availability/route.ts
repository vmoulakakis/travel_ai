...
export async function POST(request:Request){
 try{
  ...
  const availabilityTruth=selected?assessStayAvailabilityV20(selected,startDate,endDate):null;
  const alternativeTruth=alternatives.map(offer=>({sourceProductId:offer.sourceProductId,...assessStayAvailabilityV20(offer,startDate,endDate)}));
  return NextResponse.json({version:20,status:selected?"FEED_CONFIRMED":"UNCONFIRMED",checkedAt:new Date().toISOString(),calendar:tripCalendarKind(startDate,endDate),availabilityTruth,alternativeTruth,stayRequirements:{hard:stayRequirements.hard,soft:stayRequirements.soft},selected,alternatives,windows},{headers:{"cache-control":"no-store"}});
 }catch{return NextResponse.json({version:20,status:"UNAVAILABLE",message:"Live feed check unavailable"},{status:503,headers:{"cache-control":"no-store"}})}
}
