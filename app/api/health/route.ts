...
function deploymentMetadata(){
 const platform=process.env.DEPLOYMENT_PLATFORM??(process.env.VERCEL?"vercel":"node");
 const commit=(process.env.DEPLOYMENT_COMMIT??process.env.GITHUB_SHA??process.env.VERCEL_GIT_COMMIT_SHA??"unknown").slice(0,8);
 const environment=process.env.DEPLOYMENT_ENV??process.env.VERCEL_ENV??process.env.NODE_ENV??"unknown";
 return{platform,commit,environment};
}
